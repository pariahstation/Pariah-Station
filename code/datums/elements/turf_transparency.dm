...
/datum/element/turf_z_transparency/proc/add_baseturf_underlay(turf/our_turf)
	if(is_openspace) // we don't ever want our bottom turf to be openspace
		our_turf.ChangeTurf(/turf/simulated/open/floor/plating, flags = CHANGETURF_INHERIT_AIR)
	var/turf/path = SSmapping.level_trait(our_turf.z, ZTRAIT_BASETURF) || /turf/simulated/open/space
	if(!ispath(path))
		path = text2path(path)
		if(!ispath(path))
