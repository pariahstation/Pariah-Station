...
/turf/simulated/open/openspace/airless
	initial_gas_mix = AIRLESS_ATMOS

/turf/simulated/open/openspace/airless/planetary
	planetary_atmos = TRUE

/turf/simulated/open/openspace/Initialize(mapload) // handle plane and layer here so that they don't cover other obs/turfs in Dream Maker
	. = ..()
...
/turf/simulated/open/openspace/LateInitialize()
	. = ..()
	AddElement(/datum/element/turf_z_transparency, is_openspace = TRUE)

/turf/simulated/open/openspace/ChangeTurf(path, list/new_baseturfs, flags)
 ...
 */
/turf/simulated/open/openspace/Enter(atom/movable/movable, atom/oldloc)
	. = ..()
	if(.)
...
/turf/simulated/open/openspace/Entered(atom/movable/movable)
	. = ..()
 ...
 */
/turf/simulated/open/openspace/proc/on_atom_created(datum/source, atom/created_atom)
	SIGNAL_HANDLER
...
/turf/simulated/open/openspace/proc/zfall_if_on_turf(atom/movable/movable)
	if(QDELETED(movable) || movable.loc != src)
		return
	zFall(movable)

/turf/simulated/open/openspace/can_have_cabling()
	if(locate(/obj/structure/lattice/catwalk, src))
		return TRUE
	return FALSE

/turf/simulated/open/openspace/zAirIn()
	return TRUE

/turf/simulated/open/openspace/zAirOut()
	return TRUE

/turf/simulated/open/openspace/zPassIn(atom/movable/A, direction, turf/source)
	if(direction == DOWN)
...
/turf/simulated/open/openspace/zPassOut(atom/movable/A, direction, turf/destination)
	if(A.anchored)
		return FALSE
	if(direction == DOWN)
		for(var/obj/contained_object in contents)
...
/turf/simulated/open/openspace/proc/CanCoverUp()
	return can_cover_up

/turf/simulated/open/openspace/proc/CanBuildHere()
	return can_build_on

/turf/simulated/open/openspace/attackby(obj/item/C, mob/user, params)
	..()
	if(!CanBuildHere())
		return
...
/turf/simulated/open/openspace/build_with_floor_tiles(obj/item/stack/tile/iron/used_tiles)
	if(!CanCoverUp())
		return
	return ..()

/turf/simulated/open/openspace/rcd_vals(mob/user, obj/item/construction/rcd/the_rcd)
	if(!CanBuildHere())
		return FALSE
...
/turf/simulated/open/openspace/rcd_act(mob/user, obj/item/construction/rcd/the_rcd, passed_mode)
	switch(passed_mode)
		if(RCD_FLOORWALL)
...
/turf/simulated/open/openspace/rust_heretic_act()
	return FALSE

/turf/simulated/open/openspace/CanAStarPass(obj/item/card/id/ID, to_dir, atom/movable/caller)
...
/turf/simulated/open/openspace/icemoon
	name = "ice chasm"
	baseturfs = /turf/simulated/open/openspace/icemoon
	initial_gas_mix = ICEMOON_DEFAULT_ATMOS
	planetary_atmos = TRUE
	var/replacement_turf = /turf/simulated/open/misc/asteroid/snow/icemoon
...
/turf/simulated/open/openspace/icemoon/Initialize(mapload)
	. = ..()
	var/turf/T = below()
	...
	M.mineralAmt = 0
	M.gets_drilled()
	baseturfs = /turf/simulated/open/openspace/icemoon //This is to ensure that IF random turf generation produces a openturf, there won't be other turfs assigned other than openspace.

/turf/simulated/open/openspace/icemoon/keep_below
	drill_below = FALSE

/turf/simulated/open/openspace/icemoon/ruins
	protect_ruin = FALSE
	drill_below = FALSE
