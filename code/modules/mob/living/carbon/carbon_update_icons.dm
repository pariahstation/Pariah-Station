...
/mob/living/carbon/proc/update_body_parts(var/update_limb_data)
	...
	if(!needs_update.len && !limb_count_update) //PARIAH MODULAR EDIT END
		return
	...
	for(var/obj/item/bodypart/BP as anything in bodyparts) //PARIAH MODULAR EDIT START
		if(BP in needs_update)
			var/bp_icon = BP.get_limb_icon()
			new_limbs += bp_icon
			limb_icon_cache[icon_render_keys[BP.body_zone]] = bp_icon
		else
			new_limbs += limb_icon_cache[icon_render_keys[BP.body_zone]] //PARIAH MODULAR EDIT END
	if(new_limbs.len)
