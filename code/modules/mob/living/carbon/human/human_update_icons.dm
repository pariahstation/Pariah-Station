...
/mob/living/carbon/human/update_inv_w_uniform()
	...
	if(istype(w_uniform, /obj/item/clothing/under))
		...
		if(OFFSET_ACCESSORY in dna.species.offset_features)
			for(var/mutable_appearance/overlay as anything in U.accessory_overlays)
				if(!istype(overlay))
					stack_trace("[__FILE__] [__LINE__] - accessory_overlays contained an invalid entry")
					continue
				overlay.pixel_x += dna.species.offset_features[OFFSET_ACCESSORY][1]
				overlay.pixel_y += dna.species.offset_features[OFFSET_ACCESSORY][2]
