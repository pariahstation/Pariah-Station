...
/obj/item/organ/heart/gland/heal/activate()
	...
	for(var/zone in limb_list)
		...
		if(!limb)
			replace_limb(zone)
			return
		if((limb.get_damage() >= (limb.max_damage / 2)) || (!IS_ORGANIC_LIMB(limb))) //PARIAH MODULAR EDIT
	...
	if((chest.get_damage() >= (chest.max_damage / 4)) || (!IS_ORGANIC_LIMB(chest))) //PARIAH MODULAR EDIT
		replace_chest(chest)
		return
...
/obj/item/organ/heart/gland/heal/proc/replace_chest(obj/item/bodypart/chest/chest)
	if(!IS_ORGANIC_LIMB(chest)) //PARIAH MODULAR EDIT
