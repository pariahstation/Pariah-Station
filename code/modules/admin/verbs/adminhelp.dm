...
/datum/admin_help_tickets/proc/stat_entry()
	...
	for(var/I in active_tickets)
		...
		if(AH.initiator)
			...
			L[++L.len] = list("[AH.handler ? "H-[AH.handler]" : ""]#[AH.id]. [AH.initiator_key_name]:", "[updated.name]", REF(AH))
		else
			++num_disconnected
	if(num_disconnected)
 ...
 */
/datum/admin_help/New(msg_raw, client/C, is_bwoink, client/admin_C, urgent = FALSE)
	//clean the input msg
	...
	if(is_bwoink)
		...
		AddInteractionPlayer("<font color='blue'>[key_name_admin(usr, FALSE)] PM'd [LinkedReplyName()]</font>")
...
/datum/admin_help/proc/Reopen()
	...
	AddInteractionPlayer("<font color='purple'>Reopened by [key_name_admin(usr, FALSE)]</font>")
...
/datum/admin_help/proc/Close(key_name = key_name_admin(usr), silent = FALSE)
	...
	AddInteractionPlayer("<font color='red'>Closed by [key_name_admin(usr, FALSE)].</font>")
	if(!silent)
		SSblackbox.record_feedback("tally", "ahelp_stats", 1, "closed")
...
/datum/admin_help/proc/Resolve(key_name = key_name_admin(usr), silent = FALSE)
	...
	AddInteractionPlayer("<font color='green'>Resolved by [key_name_admin(usr, FALSE)].</font>")
...
/datum/admin_help/proc/Reject(key_name = key_name_admin(usr))
	...
	AddInteractionPlayer("Rejected by [key_name_admin(usr, FALSE)].")
...
/datum/admin_help/proc/ICIssue(key_name = key_name_admin(usr))
	...
	AddInteractionPlayer("Marked as IC issue by [key_name_admin(usr, FALSE)]")
...
/datum/admin_help_ui_handler/proc/perform_adminhelp(client/user_client, message, urgent)
	...
	new /datum/admin_help(message, user_client, FALSE, urgent=urgent)
...
/proc/admin_ticket_log(what, message, log_in_blackbox = TRUE, admin_only = TRUE)
	var/client/C
	var/mob/Mob = what
	if(istype(Mob))
		C = Mob.client
