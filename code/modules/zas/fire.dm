...
/datum/gas_mixture/proc/react(zone/zone, force_burn, no_check = 0)
	...
	if((temperature > PHORON_MINIMUM_BURN_TEMPERATURE || force_burn) && (no_check ||check_recombustability(zone? zone.fuel_objs : null)))
		...
		#ifdef FIREDBG
		log_game("***************** FIREDBG *****************")
		log_game("Burning [zone? zone.name : "zoneless gas_mixture"]!")
		#endif
		...
		#ifdef FIREDBG
		log_game("gas_fuel = [gas_fuel], liquid_fuel = [liquid_fuel], total_oxidizers = [total_oxidizers]")
		log_game("fuel_area = [fuel_area], total_fuel = [total_fuel], reaction_limit = [reaction_limit]")
		log_game("firelevel -> [firelevel] (gas: [gas_firelevel], liquid: [liquid_firelevel])")
		log_game("liquid_reaction_progress = [liquid_reaction_progress]")
		log_game("gas_reaction_progress = [gas_reaction_progress]")
		log_game("total_reaction_progress = [total_reaction_progress]")
		log_game("used_fuel = [used_fuel], used_oxidizers = [used_oxidizers]; ")
		#endif
		...
		#ifdef FIREDBG
		log_game("used_gas_fuel = [used_gas_fuel]; used_liquid_fuel = [used_liquid_fuel]; total = [used_fuel]")
		log_game("new temperature = [temperature]; new pressure = [return_pressure()]")
		#endif
...
/datum/gas_mixture/proc/check_combustability(obj/effect/decal/cleanable/liquid_fuel/liquid=null)
	. = 0
	for(var/g in gas)
		if(gas_data.flags[g] & XGM_GAS_OXIDIZER && QUANTIZE(gas[g] * SSzas.settings.fire_consuption_rate) >= 0.1)
			. = 1
			break
	...
	. = 0
	for(var/g in gas)
		if(gas_data.flags[g] & XGM_GAS_FUEL && QUANTIZE(gas[g] * SSzas.settings.fire_consuption_rate) >= 0.1)
			. = 1
			break
...
/atom/proc/FireBurn(firelevel, last_temperature, pressure)
	return

/mob/living/FireBurn(firelevel, last_temperature, pressure)
...
/mob/living/carbon/human/FireBurn(firelevel, last_temperature, pressure)
	...
	for(var/obj/item/clothing/C in get_all_worn_items())
	...
	return mx * (head_exposure + chest_exposure + legs_exposure + arms_exposure)
