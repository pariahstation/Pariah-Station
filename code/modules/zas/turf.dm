...
/turf/simulated/update_air_properties()
	...
	if(s_block & AIR_BLOCKED)
		#ifdef ZASDBG
		if(verbose) log_game("Self-blocked.")
		//dbg(blocked)
		#endif
		if(zone)
			var/zone/z = zone
	...
	for(var/d = 1, d < 16, d *= 2)
		...
		if(block & AIR_BLOCKED)
			...
			#ifdef ZASDBG
			if(verbose) log_game("[d] is blocked.")
			//unsim.dbg(air_blocked, turn(180,d))
			#endif
		...
		if(istype(unsim, /turf/simulated))
			...
			if(TURF_HAS_VALID_ZONE(sim))
				...
				if(!zone)
					...
					if(((block & ZONE_BLOCKED) && !(r_block & ZONE_BLOCKED)) || ((r_block & ZONE_BLOCKED) && !(s_block & ZONE_BLOCKED)))
						#ifdef ZASDBG
						if(verbose) log_game("[d] is zone blocked.")
						...
						#ifdef ZASDBG
						dbg(assigned)
						if(verbose) log_game("Added to [zone]")
						#endif
			...
			#ifdef ZASDBG
				else if(verbose) log_game("[d] has same zone.")

			else if(verbose) log_game("[d] has invalid zone.")
			#endif
