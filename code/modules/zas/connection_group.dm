...
/connection_edge/proc/add_connection(connection/c)
	coefficient++
	if(c.direct()) direct++
//	log_game("Connection added: [type] Coefficient: [coefficient]")


/connection_edge/proc/remove_connection(connection/c)
//	log_game("Connection removed: [type] Coefficient: [coefficient-1]")
...
/connection_edge/zone/New(zone/A, zone/B)
	...
	B.edges.Add(src)
	//id = edge_id(A,B)
//	log_game("New edge between [A] and [B]")
...
/connection_edge/unsimulated/recheck()
	...
	if(!A.air.compare(air, vacuum_exception = 1))
		SSzas.settings.mark_edge_active(src)
