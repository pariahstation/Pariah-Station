...
/datum/buildmode_mode/basic/handle_click(client/c, params, obj/object)
	...
	if(istype(object,/turf) && left_click && !alt_click && !ctrl_click)
		...
		if(isplatingturf(object))
			clicked_turf.PlaceOnTop(/turf/simulated/open/floor/iron, flags = CHANGETURF_INHERIT_AIR)
		else if(isfloorturf(object))
			clicked_turf.PlaceOnTop(/turf/closed/wall)
		else if(iswallturf(object))
			clicked_turf.PlaceOnTop(/turf/closed/wall/r_wall)
		else
			clicked_turf.PlaceOnTop(/turf/simulated/open/floor/plating, flags = CHANGETURF_INHERIT_AIR) // Gotta do something
