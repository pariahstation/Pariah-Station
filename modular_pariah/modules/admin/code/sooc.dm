...
/client/verb/sooc(msg as text)
	...
	for(var/iterated_listener as anything in listeners)
		...
		to_chat(iterated_client, span_ooc("<font color='[color]'><b><span class='prefix'>SOOC:</span> <EM>[name]:</EM> <span class='message linkify'>[msg]</span></b></font>"))
...
/proc/toggle_sooc(toggle = null)
	...
	for(var/iterated_listener as anything in listeners)
		var/client/iterated_client = iterated_listener
		to_chat(iterated_client, span_ooc("<b>The SOOC channel has been globally [GLOB.sooc_allowed ? "enabled" : "disabled"].</b>"))
